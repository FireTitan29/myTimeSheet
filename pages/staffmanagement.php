...
<div>
    <?php foreach ($staffMembers as $person): ?>
            <input type="radio" name='staffMember' id="<?= $person['staffID'] ?>" value="<?= $person['staffID'] ?>">
            <label for="<?= $person['staffID'] ?>"><?= $person['staffName']?></label> <br>
    <?php endforeach; ?>
    <p>Staff Count: <?= $staffCount ?></p>
</div>
<br>
<button class="form-button" onclick="openForm()">Add Staff</button>
